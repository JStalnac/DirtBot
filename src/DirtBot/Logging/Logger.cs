...
namespace DirtBot.Logging
{
    ...
    {
        static readonly ManagedFile logFile;
        ...
        public static void Log(string message, bool writeFile = false, Exception exception = null, ConsoleColor fore = ConsoleColor.Gray, ConsoleColor back = ConsoleColor.Black)
        {
            StackFrame frame = new StackTrace().GetFrame(1);
            string methodString = GetMethodString(frame.GetMethod());

            LogInternal(source: methodString, message: message, writeFile: writeFile, exception: exception, foregroundColor: fore, backgroundColor: back);
        }

        private static Task LogInternal(string source, string message, bool writeFile, Exception exception, ConsoleColor foregroundColor, ConsoleColor backgroundColor)
        {
